...
| 6 | Medium | `app/core/database.py:88-139` | `get_db()`'s `except` only catches `psycopg2.Error`; any other exception (e.g. `HTTPException`) skips rollback and falls through to an unconditional `conn.commit()` in `finally`. `get_db_transaction()` (same file) correctly catches broad `Exception`. No live write path currently uses `get_db()`, but it's a footgun if one ever does. **Fixed (2026-09-24)** — commits only when the block finishes; any exception rolls back (a failed rollback keeps the original error); tests in `tests/unit/core/test_get_db.py`, which CI runs (`tests/test_database.py` is outside `testpaths`). |
| 7 | Medium | `app/main.py:55-70` | `allow_origin_regex` (`new-look.*\.vercel\.app` etc.) + `allow_credentials=True` — Vercel lets anyone deploy a project under an arbitrary name matching that pattern (e.g. `new-look-anything.vercel.app`), granting credentialed cross-origin access. **Assessed (2026-09-24), low in practice:** sign-in travels as an `Authorization: Bearer` header the frontend attaches, and the API sets no cookies, so a matching origin gains no ambient credentials — it can read only what any client can. Left as is so team preview deployments keep working; tighten the regex to exact project names if previews stop being used. |
| 8 | Medium | `app/middleware/rate_limit.py:25-40` | `get_client_ip` trusts a client-supplied `X-Forwarded-For` unconditionally — an attacker can bypass `login_limiter`'s 3/min brute-force throttle on `/auth/login` by varying the header per request. (The separate global limiter in `middleware/rate_limiter.py` correctly uses `request.client.host` — inconsistency between the two layers.) **Fixed (2026-09-24)** — `get_client_ip` returns `request.client.host`, which uvicorn (proxy headers on by default, trusting 127.0.0.1) already resolves from the right end of X-Forwarded-For; tests show a forged header no longer changes the bucket. |
| 9 | Medium | `app/api/v1/endpoints/analysis.py:52-84` and `88-120` | `FRONTEND_CODE_TO_STREAM` dict is defined twice, verbatim, back to back — a landmine if one copy is edited without the other. **Fixed (2026-09-24)** — the second copy (identical, checked with `ast.literal_eval`) is removed. |
...
| 11 | Low-Med | 46 occurrences across `analysis.py`, `codigestion.py`, `geospatial.py`, `intermediate_regions.py`, `mock_geospatial.py`, `municipalities.py`, `proximity.py`, `residuos.py`, `scientific.py`, `technology_routes.py`, `mapbiomas_service.py` | Raw exception text (`detail=f"...{str(e)}"`) leaked into HTTP responses — inconsistent with the more defensive generic-message pattern used elsewhere in the same files. **Fixed (2026-09-24)** — every handler answers with a fixed message and logs the exception (18 that logged nothing now use `logger.exception`); the land-use part of a proximity analysis no longer carries the exception text either; `mock_geospatial.py` (unused sample-data router, mounted in production) is removed. `tests/unit/test_error_details.py` scans `app/` so the pattern cannot come back. |
| 12 | Low | `app/services/proximity_service.py:193` | Re-reads and re-parses the full state shapefile from disk on every `/proximity/analyze` call, unlike `geospatial.py`'s `_load_geo_gdf()` which caches it module-level. **Fixed (2026-09-24)** — `read_shapefile_wgs84()` reads and reprojects each shapefile (municipalities and infrastructure layers) once per process; a test counts the disk reads. |
...
| 5 | Medium | `ComparisonPanel.tsx`, `ExportControl.tsx`, `CodigestionDetailPanel.tsx` | Modal-style panels with no `role="dialog"`/`aria-modal`, no focus trap, no component-level Escape handling (the only Escape listener is global, in `MapComponent.tsx`, and only closes municipality selection). **Fixed (2026-09-24)** — ComparisonPanel and ExportControl are modal dialogs on `useDialog` (role, aria-modal, aria-labelledby; Escape closes, focus moves in and returns); CodigestionDetailPanel, a non-modal side panel beside the live map, is a labelled `<section>`. Tests assert the dialog names and Escape. |
| 6 | Medium | `components/map/MapComponent.tsx:291-306` | Enabling the intermediate-regions layer force-sets `mapScope` to `'brazil'`; disabling it again never restores `'sp'` — map stays zoomed out with no visible reason. **Fixed (verified 2026-09-24)** — the regions layer switch only enables its data (`setIntermediateRegionsEnabled`); the scope is no longer touched. |
| 7 | Medium | `app/[locale]/dashboard/scientific-database/page.tsx:189-346,480-490` | Unconditional `setInterval(fetchAllData, 30000)` re-runs ~8 API calls every 30s regardless of tab visibility, plus no de-dup against the manual refresh button. **Fixed (verified 2026-09-24)** — the scientific database was rebuilt on `useScientificDatabase`, which loads on demand; there is no interval. |
| 8 | Medium | `components/map/InfrastructureLayer.tsx:317-417`, `CodigestionClusterLayer.tsx:86-98` | Popup/tooltip HTML built via raw string interpolation into `bindPopup()`/`bindTooltip()` — low exploitability today (source is static shapefiles/backend-computed labels) but a stored-XSS vector waiting for any future user-editable field. **Fixed (2026-09-24)** — `lib/html.ts` `escapeHtml` is used by every HTML string built for Leaflet: InfrastructureLayer (which had its own copy), MunicipalityLayer, BubbleChartLayer, both intermediate-region layers and CodigestionClusterLayer. |
