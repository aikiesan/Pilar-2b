...
_Started: 2026-07-01. Last updated: 2026-07-01 (Round 3 code review)._
...
| Frontend `test:e2e:public` | 🔴 **Still red — see "Open decision" section above.** Round 3 initially believed a PR #134 run was green (`conclusion: success`) and removed `continue-on-error`, but that reading only reflected the field being masked by `continue-on-error` still being active on that run — the actual logs show 5 failed tests. PR #135's run: 3 failed + 6 flaky, same root cause (CORS against the live production backend). | **Not fixed.** Gate is currently hard-blocking on a genuinely non-green suite — needs either a non-production E2E backend target or `continue-on-error` restored until that's done. |
...
## ⚠️ Open decision: `frontend-e2e` CI gate is currently red
...
**Not yet fixed or reverted** — a `continue-on-error: true` edit was drafted
but never committed. The gate is presently hard-blocking and will fail on
every PR until one of these happens:
- Point E2E at a non-production backend (local/staging with permissive CORS
  for the test origin), or
- Soften the gate back to `continue-on-error: true` until that's done.
...
| 2 | High | `app/routers/technology_routes.py:239` | `get_technology_by_id` joins a table literally named `references`, which no migration creates (only `residuo_references`/`technology_references`/`scientific_references` exist) — sibling endpoint `get_all_technologies` correctly uses `residuo_references`. This endpoint 500s on every call. |
| 3 | High | `app/api/v1/endpoints/codigestion.py:17-22,58-64,91-102,180-190` | `_cluster_cache`/`_cn_profile_cache` are unbounded module-level dicts (no size/TTL limit, unlike the bounded `LRUCache` in `services/cache_service.py`); `min_biomass_tons` has no upper bound so a caller can mint unlimited cache entries; `DELETE /clusters/cache` has no auth either. Unauthenticated memory-exhaustion vector. |
| 4 | Med-High | `app/routers/technology_routes.py:149-162` | N+1 query in `get_all_technologies` — one extra `SELECT` per row instead of a single batched `IN (...)` query. |
| 5 | Med-High | `app/services/proximity_service.py:503-523,577-578` | Same N+1 pattern in `correlate_mapbiomas_residuos` (one query per land-use class), wrapped in a bare `except Exception: log` with no re-raise — a partial DB failure returns a 200 with silently incomplete data. |
...
| # | Severity | Where | Problem |
|---|---|---|---|
| 1 | High | `app/[locale]/dashboard/advanced-analysis/page.tsx:348-428` | `fetchAllData`'s debounced filter-change effect has no request-id/AbortController guard — two filter changes within one round-trip can race, silently showing stats for a filter combo the user isn't looking at anymore. Same bug class as the retry-after-error fix this session, unaddressed here. |
| 2 | High | `app/[locale]/dashboard/proximity/page.tsx:123-151` | `handleMapClick` clears state but doesn't cancel the in-flight `analyzeProximity` request — clicking a second point while the first is still loading can display point-A's results while point-B is shown as selected. |
| 3 | High | `components/map/MunicipalityLayer.tsx:195-213` | Popup `createRoot(container)` per popup open, never `root.unmount()`'d — leaks a React root per municipality click over a long session; only spot in the codebase with this pattern. |
| 4 | High | `components/map/ExportControl.tsx:170-200` | `exportToPNG` doesn't await/wrap `canvas.toBlob(callback)` — the "Download iniciado!" success toast fires before the blob/link/click actually run, so a slow or failed render still shows success with no file downloaded; the `throw` inside the callback also escapes the surrounding `try/catch`. |
