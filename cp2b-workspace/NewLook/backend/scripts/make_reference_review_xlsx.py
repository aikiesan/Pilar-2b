...
U = importlib.util.module_from_spec(_u); _u.loader.exec_module(U)  # reuse CODE_MAP

DOI_RE = re.compile(r'^10\.', re.I)
BMP_RE = re.compile(r'(\d{2,4}(?:[.,]\d+)?)\s*(?:±\s*[\d.,]+\s*)?(?:m?[lL]|Nm?[lL]|L)\s*(?:CH[₄4])?\s*/\s*(?:kg|g)\s*(?:VS|SV)', re.I)
...
def main(src, out='PILAR2b_referencias_revisao.xlsx'):
    ...
    headers = ['id', 'primary_residue', 'feedstock_canonico', 'title', 'authors', 'journal', 'year',
               'reference_type', 'validation_status', 'current_url', 'current_doi', 'suspect_doi_reuse',
               'needs_url', 'bmp_extracted', 'notes_excerpt', 'CORRECTED_URL', 'CORRECTED_DOI',
               'STATUS', 'REVIEWER_NOTES']
    edit_cols = {'CORRECTED_URL', 'CORRECTED_DOI', 'STATUS', 'REVIEWER_NOTES'}

    wb = Workbook(); ws = wb.active; ws.title = 'References (399)'
    ws.append(headers)
    ...
    data = []
    for r in rows:
        feed = U.CODE_MAP.get((r.get('primary_residue') or '').strip(), (r.get('primary_residue') or '').upper())
        d = clean_doi(r.get('doi'))
        ...
        data.append((feed, rid, [r.get('id'), r.get('primary_residue'), feed, r.get('title'), r.get('authors'),
                     r.get('journal'), r.get('year'), r.get('reference_type'), r.get('validation_status'),
                     url, d, suspect, needs, (bmps[0] if bmps else ''), note[:200], '', '', '', '']))
    data.sort(key=lambda x: (x[0], x[1]))

    red = PatternFill('solid', fgColor='FDE2E2'); amber = PatternFill('solid', fgColor='FEF3C7')
    edit = PatternFill('solid', fgColor='FFF7CC')
    for _feed, _id, row in data:
        ws.append(row); ridx = ws.max_row
        if row[11] == 'Y':
            for c in range(1, len(headers) + 1): ws.cell(ridx, c).fill = red
        elif row[12] == 'Y':
            for c in range(1, len(headers) + 1): ws.cell(ridx, c).fill = amber
        for c in (16, 17, 18, 19): ws.cell(ridx, c).fill = edit

    widths = {'A': 6, 'B': 22, 'C': 18, 'D': 46, 'E': 30, 'F': 24, 'G': 6, 'H': 22, 'I': 16,
              'J': 46, 'K': 30, 'L': 8, 'M': 8, 'N': 10, 'O': 50, 'P': 40, 'Q': 28, 'R': 12, 'S': 30}
    for col, w in widths.items(): ws.column_dimensions[col].width = w
    ws.freeze_panes = 'A2'
    ws.auto_filter.ref = f'A1:{get_column_letter(len(headers))}{ws.max_row}'
    dv = DataValidation(type='list', formula1='"OK,FIXED_URL,FIXED_DOI,FIXED_BOTH,REMOVE,KEEP_NO_DOI"', allow_blank=True)
    ...
    wb.save(out)
    print(f'rows={len(data)} suspect={sum(1 for d in data if d[2][11] == "Y")} '
          f'needs_url={sum(1 for d in data if d[2][12] == "Y")} -> {out}')


if __name__ == '__main__':
    main(sys.argv[1] if len(sys.argv) > 1 else 'scientific_references.csv',
