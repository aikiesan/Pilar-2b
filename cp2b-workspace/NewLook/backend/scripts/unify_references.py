...
def main(path):
    ...
    seen, canon = {}, []
    for r in rows:
        code = r.get('codigo_residuo') or r.get('primary_residue') or ''
        feed = CODE_MAP.get(code, code)
        doi = clean_doi(r.get('doi'))
        url = clean(r.get('url'))
        url = re.sub(r'[>"<)\s]+$', '', url) if url else None
        if not url and doi: url = f'https://doi.org/{doi}'
        if not url: url = url_from_text(r.get('referencia_texto'))
        text = clean(r.get('referencia_texto')) or clean(r.get('titulo')) or clean(r.get('autores')) or ''
        ...
        rec = dict(codes={feed}, ids=[r.get('id','')], citation=text[:300], url=url, doi=doi,
                   year=clean(r.get('ano')), peer=clean(r.get('tipo_fonte')) in PEER, needs_url=not bool(url))
    ...
    with out.open('w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(['feedstock_codes','citation','url','doi','year','peer_reviewed','needs_url','source_ids'])
        for r in sorted(canon, key=lambda x: sorted(x['codes'])):
            w.writerow(['|'.join(sorted(r['codes'])), r['citation'], r['url'] or '', r['doi'] or '',
                        r['year'] or '', r['peer'], r['needs_url'], ';'.join(map(str, r['ids']))])
