...
CODE_MAP = {
    "AG_CANA_BAGACO": "BAGACO",
    "AG_CANA_TORTA_FILTRO": "TORTA_FILTRO",
    "AG_CANA_VINHACA": "VINHACA",
    "AG_CAFE_CASCA": "CASCA_CAFE",
    "AG_CAFE_POLPA": "POLPA_CAFE",
    "AG_CAFE_MUCILAGEM": "MUCILAGEM_CAFE",
    "AG_CITROS_BAGACO": "BAGACO_CITROS",
    "AG_CITROS_CASCAS": "CASCAS_CITROS",
    "AG_CITROS_POLPA": "BAGACO_CITROS",
    "AG_MILHO_PALHA": "PALHA_MILHO",
    "AG_SOJA_PALHA": "PALHA_SOJA",
    "PEC_CAMA_AVIARIO": "CAMA_AVIARIO",
    "PEC_DEJETOS_FRESCOS_AVES": "DEJETOS_AVES",
    "PEC_DEJETOS_LIQUIDOS_BOVINO": "DEJETOS_BOVINO",
    "PEC_DEJETOS_LIQUIDOS_SUINO": "DEJETOS_SUINO",
    "PEC_ESTERCO_BOVINO": "ESTERCO_BOVINO",
    "PEC_ESTERCO_SOLIDO_SUINO": "ESTERCO_SUINO",
    "PEC_GORDURA_SEBO": "GORDURA",
    "PEC_SANGUE_ANIMAL": "SANGUE",
    "CARCACAS_AVES": "CAMA_AVIARIO",
    "URB_FORSU_SEPARADA": "FORSU",
    "URB_FRACAO_ORGANICA": "ORGANICO_RSU",
    "URB_LODO_PRIMARIO": "LODO_PRIMARIO",
    "URB_LODO_SECUNDARIO": "LODO_SECUNDARIO",
    # scientific_references primary_residue scheme (lowercase)
    "bagaco_cana": "BAGACO",
    "palha_cana": "PALHA",
    "vinhaca_cana": "VINHACA",
    "torta_filtro": "TORTA_FILTRO",
    "bagaco_citros": "BAGACO_CITROS",
    "cascas_citros": "CASCAS_CITROS",
    "cascas_citros_ind": "CASCAS_CITROS",
    ...
    "dejetos_aves_frescos": "DEJETOS_AVES",
    "carcacas_aves": "CAMA_AVIARIO",
    "dejetos_bovinos_liquidos": "DEJETOS_BOVINO",
    "esterco_bovino": "ESTERCO_BOVINO",
    "dejetos_suinos": "DEJETOS_SUINO",
    "dejetos_suinos_liquidos": "DEJETOS_SUINO",
    "esterco_solido_suino": "ESTERCO_SUINO",
    "esterco_suino": "ESTERCO_SUINO",
    "gordura_sebo": "GORDURA",
    "sangue_animal": "SANGUE",
    "visceras_abatedouro": "VISCERAS",
    "forsu_ur_rsu": "FORSU",
    "fracao_organica_rsu": "ORGANICO_RSU",
    "lodo_primario_ete": "LODO_PRIMARIO",
    "lodo_secundario_ete": "LODO_SECUNDARIO",
    "soro_queijo": "SORO_QUEIJO",
    "levedura_residual": "LEVEDURA",
    "casca_eucalipto": "CASCA_EUCALIPTO",
    "bagaco_malte": "BAGACO_MALTE",
    "residuo_abatedouro": "VISCERAS",
}
...
def main(path):
    ...
    for r in rows:
        ...
        if not url:
            url = url_from_text(r.get("referencia_texto") or r.get("notes"))
        if doi:
            doi_resid.setdefault(doi, set()).add(feed)
        text = (
            clean(r.get("titulo"))
            or clean(r.get("title"))
            or clean(r.get("referencia_texto"))
            or clean(r.get("autores"))
            or clean(r.get("authors"))
            or ""
        )
        key = (
            ("doi", doi.lower()) if doi else ("txt", re.sub(r"\W+", " ", text[:60]).lower().strip())
        )
        if key in seen:
            seen[key]["codes"].add(feed)
            seen[key]["ids"].append(r.get("id", ""))
            if not seen[key]["url"] and url:
                seen[key]["url"] = url
            continue
        ...
        rec = dict(
            ...
            url=url,
            doi=doi,
            year=clean(r.get("ano")) or clean(r.get("year")),
            peer=peer,
    ...
    print(
        f"input={len(rows)} unique={len(canon)} merged={len(rows)-len(canon)} "
