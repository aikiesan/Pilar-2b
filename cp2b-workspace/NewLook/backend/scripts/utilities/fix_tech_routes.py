...
# Read the file
with open(
    "cp2b-workspace/NewLook/backend/app/routers/technology_routes.py", "r", encoding="utf-8"
) as f:
    content = f.read()
...
content = re.sub(r",\s*db: Session = Depends\(get_db\)", "", content)
...
content = re.sub(
    r"result = db\.execute\(text\((.*?)\)\)", r"cursor.execute(\1)", content, flags=re.DOTALL
)
...
# Write back
with open(
    "cp2b-workspace/NewLook/backend/app/routers/technology_routes.py", "w", encoding="utf-8"
) as f:
    f.write(content)
