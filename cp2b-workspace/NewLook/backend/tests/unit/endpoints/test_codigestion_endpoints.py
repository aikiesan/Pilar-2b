...
FAKE_CANDIDATES = [
    {"ibge_code": "3509502", "municipality_name": "Campinas", "improvement_score": 6.5, "distance_km": 12.3}
]
...
class TestGetClusters:
    ...
    def test_returns_200_with_service_mock(self, client):
        with patch("app.services.codigestion_service.find_codigestion_clusters",
...
class TestMunicipalityCNProfiles:
    ...
    def test_returns_200(self, client):
        with patch("app.services.codigestion_service.get_municipality_cn_profiles",
...
class TestResidueCNMatrix:
    ...
    def test_returns_200(self, client):
        with patch("app.services.codigestion_service.get_residue_cn_matrix",
                   return_value=FAKE_CN_MATRIX):
...
class TestPairingCandidates:
    ...
    def test_returns_200_with_valid_ibge(self, client):
        with patch("app.services.codigestion_service.get_pairing_candidates",
...
class TestGetClusterDetail:
    ...
    def test_cache_hit_returns_same_data(self, client):
        with patch("app.services.codigestion_service.find_codigestion_clusters",
                   return_value=FAKE_CLUSTERS) as mock_svc:
            client.delete("/api/v1/codigestion/clusters/cache")
