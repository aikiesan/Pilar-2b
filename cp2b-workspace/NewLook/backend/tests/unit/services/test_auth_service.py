...
from app.services.auth_service import AuthService, MAX_FAILED_LOGINS
from app.models.auth import UserLogin, AdminCreateUser
...
def test_get_current_user_rejects_wrong_signature(svc):
    token = jwt.encode(
        ...
        "a-different-secret-key-of-sufficient-length-xx", algorithm="HS256",
    )
    with pytest.raises(HTTPException) as e:
        asyncio.run(svc.get_current_user(token))
    assert e.value.status_code == 401
...
def test_login_wrong_password_raises_and_increments(svc, cursor):
    ...
    assert e.value.status_code == 401
    assert any("failed_login_count" in (sql or "") for sql, _ in cursor.executed)
...
def test_login_locks_after_max_failures(svc, cursor):
    ...
    with pytest.raises(HTTPException):
        asyncio.run(svc.login_user(UserLogin(email=row["email"], password="WrongPass1")))
    upd = [params for sql, params in cursor.executed if "locked_until" in (sql or "")]
...
def test_login_unknown_email_is_401(svc, cursor):
    ...
    with pytest.raises(HTTPException) as e:
        asyncio.run(svc.login_user(UserLogin(email="nobody@cp2b.unicamp.br", password="Password123")))
    assert e.value.status_code == 401
...
def test_create_user_returns_profile(svc, cursor):
    ...
    data = AdminCreateUser(email=created["email"], password="Password123",
                           full_name="Test User", role="autenticado", clearance=0)
