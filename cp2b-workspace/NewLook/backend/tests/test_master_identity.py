...
class TestMasterIdentity(unittest.TestCase):
    def test_canonical_master_sha256(self):
        ...
        self.assertTrue(
            os.path.exists(provenance_path), f"Provenance file missing: {provenance_path}"
        )
        ...
        self.assertEqual(
            actual_sha, expected_sha, f"SHA-256 mismatch! Expected {expected_sha}, got {actual_sha}"
        )
        ...
        for sec in secondary_paths:
            if os.path.exists(sec):
                ...
                self.assertEqual(
                    sec_sha, expected_sha, f"Secondary mirror {sec} differs from canonical SHA-256!"
                )
