...
async def get_tile(z: int, x: int, y: int):
    ...
    if not RASTER_PATH.exists():
        raise HTTPException(status_code=404, detail="MapBiomas raster file not found")

    # Limit zoom levels for performance
    if z < 5 or z > 16:
        raise HTTPException(status_code=400, detail=f"Zoom level {z} not supported. Use 5-16.")
    ...
    try:
        ...
        with rasterio.open(RASTER_PATH) as src:
            ...
            if (
                east < raster_bounds.left
                or west > raster_bounds.right
                or north < raster_bounds.bottom
                or south > raster_bounds.top
            ):
