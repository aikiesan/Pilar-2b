...
export default function LegalDocument({ version, text }: LegalDocumentProps) {
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 py-16">
      ...
      <p
        role="note"
        className="text-sm bg-amber-50 border border-amber-200 text-amber-900 dark:bg-amber-900/30 dark:border-amber-700 dark:text-amber-100 rounded-md px-3 py-2 mb-6"
      >
        {doc.draftNote}
      </p>
      <p className="text-gray-700 dark:text-slate-300 mb-8 leading-relaxed">{doc.intro}</p>

      {doc.sections.map((s) => (
        <section key={s.h} className="mb-6">
          <h2 className="text-xl font-semibold mb-2">{s.h}</h2>
          {s.p.map((para, i) => (
            <p key={i} className="text-gray-700 dark:text-slate-300 leading-relaxed mb-2">
              {para}
            </p>
          ))}
        </section>
